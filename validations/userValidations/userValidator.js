...
module.exports = {
    registerUserValidation: async (req, res, next) => {
        const value = await userValidationSchema.registerUser.validate(req.body, { abortEarly: false })
        if (value.error) {
    ...
    loginUserValidation: async (req, res, next) => {
        const value = await userValidationSchema.loginUser.validate(req.body, { abortEarly: false })
        if (value.error) {
            return res.status(403).json({
                success: false,
                message: value.error.details[0].message
